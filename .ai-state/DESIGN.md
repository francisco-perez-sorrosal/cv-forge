...
| data-source | `CvDataSnapshot` (immutable, parsed at the boundary), `CvDataProvider` (the one mutable holder, owns refresh and state), `ReleaseFetcher`/`ArtifactCache` (GitHub release access), `load_local_dir`. The only code that knows about filesystem paths or sockets. See `dec-draft-45aafe55`. `snapshot.py`/`release.py`/`local.py` are Built as of M1.4 (`ReleaseManifest`/`AssetEntry`/`ArtifactUnavailable` are Pydantic models, matching the `Resume`/`SemanticOverlay` boundary-parsing pattern, per `LEARNINGS.md`); `provider.py` (`CvDataProvider`, `ReleaseFetcher`/`ArtifactCache`) remains Designed, pending M1.6. | Designed | `src/cv_forge/data/{snapshot,release,local}.py` (Built), `src/cv_forge/data/provider.py` (Designed) <!-- aac-override: text-only status correction (topology unchanged), no .c4/diagram regen run — see LEARNINGS.md M1.4 --> |
