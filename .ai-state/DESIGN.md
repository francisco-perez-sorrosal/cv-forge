...
| **Current as of** | `dec-008` (finalized 2026-09-14) — all eight decisions from the `cv-repo-split` pipeline are finalized records under `.ai-state/decisions/` and folded into §1–§3. |
| **Last verified** | 2026-09-14 by verifier (post-implementation, live probes against Wasmer Edge and GitHub Releases) |
...
|-----------|---------------|--------|-----------|
| models | Pydantic definitions of `Resume`, `SemanticOverlay` and `TailoringSpec` — the shape of the data and the source of the exported JSON Schemas. No I/O, no paths, no network. | Built | `src/cv_forge/models/{resume,semantics,tailoring}.py` |
| render | Jinja2 renderers and templates producing markdown, LaTeX (moderncv), HTML and Typst (moderner-cv), full and tailored. Pure functions of a store. | Built | `src/cv_forge/render/renderers.py`, `src/cv_forge/render/templates/` |
| data-store | `ResumeStore` — read-only query layer over a parsed `Resume` and `SemanticOverlay`: entry index, cross-reference validation, the query methods every tool reads through. Narrowed by this work: its loading and semantics-persistence responsibilities move out (to data-source and cli respectively). | Built | `src/cv_forge/data/store.py` <!-- aac-override: text-only status correction (topology unchanged), no .c4/diagram regen run — see LEARNINGS.md M1.4 --> |
| data-source | `CvDataSnapshot` (immutable, parsed at the boundary), `CvDataProvider` (the one mutable holder, owns refresh and state), `ReleaseFetcher`/`ArtifactCache` (GitHub release access), `load_local_dir`. The only code that knows about filesystem paths or sockets. See `dec-006`. All Built: `ReleaseFetcher` is a `Protocol` in `provider.py`, implemented by `GitHubReleaseFetcher` in `release.py` (httpx2 with a certifi-backed TLS context; on-demand PDF cache keyed by release tag); `bootstrap.py` builds the provider from the environment for both the CLI and the Edge entrypoint. | Built | `src/cv_forge/data/{snapshot,release,local,provider,bootstrap}.py` |
| mcp | `MCPServer` construction, ASGI app factory and lifespan, 17 `fps-cv://` resources, 16 tools, and the `/healthz` surface. Reads the current snapshot per call; never holds it across an await. | Built | `src/cv_forge/mcp/{server,resources,main}.py`, `src/cv_forge/mcp/tools/` (`app.py` and `/healthz` are new) |
| cli | The `cv-forge` console script: `render`, `validate`, `export-schemas`, `fetch-snapshot`, `serve`. The single render path shared by the maintainer, CI and the maintainer plugin. `render` is Built as of M1.12 (byte-identical output verified against `cv-data/`; `-f pdf` compiles via `latexmk`); the other four are registered stubs (exit 2, "not implemented in this build") so `--help` already shows the full grammar — M1.14/M1.16 fill them in. `scripts/render_cv.py` is deleted, not superseded-in-place. | Built | `src/cv_forge/cli/main.py` <!-- aac-override: text-only status correction (topology unchanged), no .c4/diagram regen run — see LEARNINGS.md M1.12 --> |
| schemas | Generated JSON Schema pair exported from the models, committed and drift-checked in CI. Source of truth for the copy mirrored into `cv`. See `dec-001`. | Built | `schemas/{resume,semantics}.schema.json` |
| publish-workflow | Reusable `workflow_call` pipeline owned by `cv-forge` and invoked by `cv`: render every format, compile the PDF, build `release.json`, upload eight assets, deploy the static site, verify it answered. | Built | `.github/workflows/publish.yml` |
| plugin-cv | Consumer-facing Claude Code plugin: native `{"type":"http"}` MCP declaration plus the `cv-analyst` and `cv-tailoring` skills. See `dec-007`. | Built | `plugins/cv/.claude-plugin/plugin.json`, `plugins/cv/skills/cv-analyst/`, `plugins/cv/skills/cv-tailoring/` <!-- aac-override: text-only path correction (topology unchanged), no .c4/diagram regen run — see LEARNINGS.md M1.21 --> |
| plugin-cv-forge | Maintainer-facing Claude Code plugin: `cv-data-edit` (edit → validate → preview → PR), `cv-publish` and `cv-forge-deploy` (both user-invocable only). No stored credentials — uses the operator's own `gh` session. | Built | `plugins/cv-forge/skills/{cv-data-edit,cv-publish,cv-forge-deploy}/**`, `plugins/cv-forge/.claude-plugin/plugin.json` |
...
| Release-sourced data serving | Serve CV content that follows `cv`'s latest release without redeploying: baked snapshot at startup, `release.json`-gated refresh, atomic swap, degradation to stale rather than failure. | Built | `data/provider.py`, `mcp/app.py` |
| One-render-path publishing | The maintainer's local preview and the CI publish run are the same code path over the same data, so local output is evidence about published output. | Built | `cli/main.py`, `.github/workflows/publish.yml` |
...
| `pydantic` | `>=2.12,<2.13.5` | Data model and validation. Upper bound is the WASIX `pydantic-core` ceiling (`2.46.4`), not a compatibility concern — see `dec-004` | Critical |
...
| No credential crosses the repository boundary | Security | Both repositories pull; neither pushes. Eliminates a long-lived cross-repo PAT and its rotation burden. See `dec-001`. |
...
Eight fragments were authored by the `cv-repo-split` pipeline: the repo-split boundary (`dec-005`), the `cv_forge` package layout (`dec-002`), runtime release-sourced data (`dec-006`), the cross-repo publish topology (`dec-001`), dropping MCPB and the registry entry (`dec-003`), the two-plugin decomposition (`dec-007`), Wasmer Edge deployment (`dec-008`), and the `mcp` 2.x SDK migration (`dec-004`).
