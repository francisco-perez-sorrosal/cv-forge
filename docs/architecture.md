...
| data-boundary | `CvDataSnapshot.parse()` (the one boundary constructor accepting raw bytes), the `LocalDir`/`ReleaseAssets`/`BakedSnapshot` origin sum type, `ReleaseManifest`/`AssetEntry`/`ArtifactUnavailable` (the `release.json` wire format, as Pydantic models), and `load_local_dir`. <!-- aac-override: new on-disk component, no .c4 model update / diagram regen run yet — see LEARNINGS.md M1.4 --> | `src/cv_forge/data/{snapshot,release,local}.py` |
